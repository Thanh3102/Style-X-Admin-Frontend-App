...
import { CreateDiscount } from "@/app/api/discount";
...
const Schema = z
  .object({
    type: z.enum(["order", "product"]),
    mode: z.enum(["coupon", "promotion"]),
    title: z
...
export type CreateDiscountData = z.infer<typeof Schema>;
...
const FormCreateDiscount = ({ type, mode }: Props) => {
  const router = useRouter();
  const [valueClass, setValueClass] = useState<string>("discount");
  const [summary, setSummary] = useImmer<Summary>({});
  const [isLoading, setIsLoading] = useState(false);
  const [openUsageLimit, setOpenUsageLimit] = useState(false);
  const [entitledProducts, setEntitledProducts] = useImmer<ProductResponse[]>(
    []
  );
  ...
  const [entitledCategories, setEntitledCategories] = useImmer<Category[]>([]);
  ...
  const {
    ...
    defaultValues: {
      ...
      startOn: today("Asia/Ho_Chi_Minh").toDate("Asia/Ho_Chi_Minh"),
      endOn: null,
      active: false,
      entitle: "all",
      entitledProductIds: [],
  ...
  const generateRandomValue = useCallback(() => {
    ...
    setValue("title", result, { shouldValidate: true });
  }, []);
  ...
  const handleUsageLimitChange = useCallback((isSelected: boolean) => {
    if (isSelected) setValue("usageLimit", 1);
    else setValue("usageLimit", null);
    setOpenUsageLimit(isSelected);
  }, []);
  ...
  const handlePrerequisiteChange = useCallback(() => {
    switch (getValues("prerequisite")) {
      ...
      case "prerequisiteMinTotal":
        ...
        setValue("prerequisiteMinTotal", 1000);
        break;
      case "prerequisiteMinItem":
        setValue("prerequisiteMinItem", 2);
        setValue("prerequisiteMinItemTotal", null);

        setValue("prerequisiteMinTotal", null);
        break;
      case "prerequisiteMinItemTotal":
        setValue("prerequisiteMinItemTotal", 1000);
        setValue("prerequisiteMinItem", null);
        setValue("prerequisiteMinTotal", null);
    }
  }, [watch("prerequisite")]);
  ...
  const onSubmit: SubmitHandler<CreateDiscountData> = async (data) => {
    try {
      setIsLoading(true);
      const session = await getSession();
      const { id, message } = await CreateDiscount(data, session?.accessToken);
      setIsLoading(false);
      toast.success(message ?? "Tạo khuyến mại thành công");
      router.push(EditDiscountRoute(id));
    } catch (error: any) {
      setIsLoading(false);
    ...
    <>
      ...
      >
        <div className="flex-[3] basis-[600px] max-w-full flex flex-col gap-4">
          ...
          <RenderIf condition={mode === "promotion"}>
          ...
          <GroupBox title="Giá trị">
            ...
            >
              ...
              <RenderIf condition={mode === "promotion" && type === "product"}>
            ...
            <RenderIf condition={type === "product"}>
              <RadioGroup
                label="Áp dụng cho"
                className="mt-2"
              ...
              <div className="mx-4">
                ...
                <div className="my-2">
                  <Checkbox
                    ...
                    onValueChange={(isSelected) =>
                      setValue("active", isSelected)
                    }
                  >
                    Kích hoạt sau khi tạo
                  </Checkbox>
                </div>
              </div>
            </GroupBox>
